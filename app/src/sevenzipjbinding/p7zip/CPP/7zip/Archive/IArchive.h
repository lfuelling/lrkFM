...
namespace NArcInfoFlags {
    ...
    const UInt32 kUseGlobalOffset =
            1 << 5;  // the seek position of stream must be set as global offset
    ...
    const UInt32 kPreArc =
            1 << 9;  // such archive can be stored before real archive (like SFX stub)
...
#define API_FUNC_static_IsArc extern "C" \
{ \
